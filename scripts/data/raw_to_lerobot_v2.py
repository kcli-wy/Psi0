...
def read_episode_data(path: Path) -> Dict:
    with open(path, "r") as f:
        data = json.load(f)
    return data
...
class HE2LeRobotConverter:
    ...
    def build_obs(self, frame: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # }
        return {
            "states": frame["states"]["qpos"] + frame["states"]["quat"]
        }
    ...
    def build_act(self, frame: Dict[str, Any]) -> List[float]:
        ...
        # print(frame.keys())
        # print(frame["states"]["qpos"].shape)
        # print(frame["states"]["qpos"])
        # exit(0)

        return frame["states"]["qpos"] + frame["states"]["quat"]
    ...
    def make_one_episode(
        ...
        try:
            ...
            data_list = read_episode_data(episode_dir / "data.json")["frames"]
            ...
            for i in range(len(data_list)-1):
                frame = data_list[i]
                next_frame = data_list[i + 1] 
                obs = self.build_obs(frame) # proprioceptive
                act = self.build_act(next_frame) # action at t, which leads to next_frame
...
def main():
    ...
    parser.add_argument("--num-workers", type=int, default=8, help="Max parallel workers (default: all CPUs)")
