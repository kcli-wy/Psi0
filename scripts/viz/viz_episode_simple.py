...
class Args:
    ...
    data_dir: str = "simple/G1WholebodyBendPick-v0-psi0"
    episode_idx: int = 0
    host: str = "0.0.0.0"
    port: int = 9000
    ...
    def __post_init__(self) -> None:
        ...
        if not Path(self.urdf).is_absolute():
            self.urdf = str(resolve_data_path(self.urdf))
...
def extract_proprio_state(frame: dict[str, Any]) -> dict[str, float]:
    # global joints_names
    # return dict(zip(joints_names, frame["observation.proprio_joint_positions"].numpy().tolist()))
    proprio_state = {}
...
def extract_action_joints(frame: dict[str, Any]) -> dict[str, float]:
    action = {}
    # global joints_names
    # action.update(dict(zip(joints_names, frame["action"].numpy().tolist()[:len(joints_names)])))
    ...
    action.update(dict(zip(LEG_JOINT_NAMES, frame["observation.leg_joints"].numpy().tolist()))) # use leg joints from observation
...
get_base_height = lambda result: -0.5 * (result["l_foot"]["position"][2] + result["r_foot"]["position"][2])  # frame["observation.prev_height"]
...
def main(args: Args) -> None:
    ...
    def _(_):
        ...
        action_qpos = extract_action_joints(frame)

        # NOTE: use finger joints from action
        for jname in joints_names[-14:]:
            joint_states[jname] = action_qpos[jname]
        ...
        # joint_states["right_hand_thumb_2_joint"] -= 0.3 # TEST
        ...
        # update z
        viser_urdf._visual_root_frame.position = (0, 0, get_base_height(result) - init_base_height) # type: ignore

        l_ee_pose = result["l_ee"]["position"]
        l_ee_pose[2] += get_base_height(result) - init_base_height
        l_ee_frame_handle.position = tuple(l_ee_pose)

        r_ee_pose = result["r_ee"]["position"]
        r_ee_pose[2] += get_base_height(result) - init_base_height
        r_ee_frame_handle.position = tuple(r_ee_pose)

        action = g1.fk(action_qpos)
        l_ee_pose_action = action["l_ee"]["position"]
        l_ee_pose_action[2] += get_base_height(result) - init_base_height
        l_ee_handle.position = tuple(l_ee_pose_action)

        r_ee_pose_action = action["r_ee"]["position"]
        r_ee_pose_action[2] += get_base_height(result) - init_base_height
    ...
    result = g1.fk(extract_proprio_state(frame))
    init_base_height = get_base_height(result) #frame["observation.prev_height"]
