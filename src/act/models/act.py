...
# from we.utils.normalize import Normalize, Unnormalize # @deprecated
...
class ACTConfig:
    ...
    state_dim: int = 15
    action_dim: int = 7
    env_state_dim: int = 10 # not used yet
...
class ACTPolicy(
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # batch = self.normalize_inputs(batch) #handle state
    ...
    def predict_action(self, batch: dict[str, Tensor]) -> Tensor:
        """Predict the full action chunk given environment observations."""
        self.eval()
        ...
        if self.config.use_vae:
            ...
            if self.config.use_robot_state:
                self.vae_encoder_robot_state_input_proj = nn.Linear(
                    self.config.state_dim, config.dim_model
                )
            # Projection layer for action (joint-space target) to hidden dimension.
            self.vae_encoder_action_input_proj = nn.Linear(
                self.config.action_dim, config.dim_model
            )
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        `batch` should have the following structure:
        {
            ...
            "observation.images": (B, n_cameras, C, H, W) batch of images.
                AND/OR
            "observation.environment_state": (B, env_state_dim) batch of environment states.
        ...
        if self.config.use_vae and "action" in batch:
            ...
            if self.config.use_robot_state:
                if robot_state_embed.dim() == 4:
                     robot_state_embed = robot_state_embed.squeeze(1)
            ...
            cls_joint_is_pad = torch.full(
                (batch_size, 2 if self.config.use_robot_state else 1),
                False,
